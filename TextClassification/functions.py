...
class tokenizer():
    ...
    def sentence_map(self,s,oobIndex):
        s_new = s.split(' ')
        s_new = [self.words_dict[word] if word in self.words_dict  else oobIndex for word in s_new]
        return s_new
    ...
    def __call__(self,sentences,padding):
        ...
        '''
        sentences_new = sentences.map(lambda x: self.sentence_map(x,self.oobIndex))
        if padding:
            max_len = max([len(x) for x in sentences])
            sentences_new = sentences_new.map(lambda x: self.sentence_padding(x,max_len,self.padIndex))
...
def batch_iter(sentences,tokenizer,ID,batch_size,sorted,label=None):
    ...
    # sentence 填充或者截断
    df = pd.DataFrame({'ID':ID,'sentences':sentences.copy(),'label':label.copy(),'seq_len':[len(x) for x in sentences]})
    ...
    for i in range(batch_num): 
        if i<batch_num-1:
            X = df.sentences[i * batch_size : ( i + 1 ) * batch_size ]
            ID = list(df.ID[i * batch_size : ( i + 1 ) * batch_size ])
            y = list(df.label[i * batch_size : (i + 1) * batch_size ]) if label is not None else None
        else:
            X = df.sentences[i * batch_size : ]
            ID = list(df.ID[i * batch_size : ])
            y = list(df.label[i * batch_size : ]) if label is not None else None
        
        X = list(tokenizer(X, padding=True))
        batch_iter.append((ID,torch.LongTensor(X),torch.LongTensor(y)))
    return batch_iter
