class FeatureEngeering:
    ...
    def VIF_filer(self, df, threshold = 10):
        ...
        while dropped:
            ...
            vif = [variance_inflation_factor(df.iloc[:,col].values,idx) for idx in range(len(col))]
            maxvif = max(vif)
            maxidx = vif.index(maxvif)
            if maxvif>threshold:
                col = col[0:maxidx]+col[maxidx+1:]
                dropped = True
...
import pandas as pd
import numpy as np
df = pd.read_csv('./58462_ldf_gaokong_18.csv')
df_train = df.iloc[0:10000, :]
df_test = df.iloc[10000:, :]
tmp = df.copy()
tmp['tmp'] = tmp.height
print(FeatureEngeering().IVCalculation(col=df.height,target=tmp.tmp>500,pos_label=True))
